...
> **Cập nhật cấu trúc kho:** Bộ spec per-category (trước ở `docs/dataset/specs/json/`) **đã được gom
> vào** `docs/dataset/catalog/catalog.jsonl` — dữ liệu spec nay nằm trong field `specs`, lọc theo
> `category.name == "Máy lạnh"`. **Nguồn gốc bất biến vẫn là** `docs/raw/Spec_cate_gia.xlsx` (sheet
> *Máy lạnh*, **1.039** dòng, **45** cột). Toàn bộ phân tích dưới đây thực hiện trên sheet gốc đó và
> giữ nguyên giá trị; lưu ý bộ "máy lạnh" trong catalog hợp nhất gồm cả sản phẩm từ `products_detail`
> nên số lượng sẽ khác 1.039.
...
- Nguồn gốc bất biến: [`docs/raw/Spec_cate_gia.xlsx`](../raw/Spec_cate_gia.xlsx), sheet *Máy lạnh* — **1.039** dòng, **45** cột.
- Dữ liệu spec sau khi gom: [`docs/dataset/catalog/catalog.jsonl`](../dataset/catalog/catalog.jsonl), field `specs`, lọc `category.name == "Máy lạnh"`.
...
Mọi số liệu dưới đây được tính trên toàn bộ sheet *Máy lạnh* của [`Spec_cate_gia.xlsx`](../raw/Spec_cate_gia.xlsx) (nay đã gom vào catalog), không lấy mẫu.
...
Bộ JSON per-category (`may-lanh.json`) đã được gom vào catalog nên các lệnh `jq` cũ trên tệp đó không
còn áp dụng. Dưới đây là cách kiểm chứng tương đương trên kho hiện tại (chạy tại gốc kho):

```sh
# Bộ "Máy lạnh" trong catalog hợp nhất = spec-only (từ xlsx) + sản phẩm từ products_detail
jq -s '[.[]|select(.category.name=="Máy lạnh")]
       | {tong:length,
          spec_only:([.[]|select(.sku!=null)]|length),
          co_ten_thuong_mai:([.[]|select(.name!=null)]|length)}' \
   docs/dataset/catalog/catalog.jsonl

# Trường bị gán nhầm nghĩa (ví dụ "Số lượng" chứa "trang A4") — vẫn tra được sau khi gom
jq -s '[.[]|select(.category.name=="Máy lạnh" and (.specs["Số lượng"]=="Khoảng 7000 trang A4"))]|length' \
   docs/dataset/catalog/catalog.jsonl

# Con số gốc 1.039 dòng / 45 cột kiểm trên nguồn bất biến:
python3 -c "import openpyxl; ws=openpyxl.load_workbook('docs/raw/Spec_cate_gia.xlsx',read_only=True)['Máy lạnh']; rows=list(ws.iter_rows(values_only=True)); print('rows(-header):',len(rows)-1,'cols:',len(rows[0]))"
```
